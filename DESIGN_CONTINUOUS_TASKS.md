...
> **Compact fire = a self-contained maintenance run (2026-07 revision).** As shipped, the compact-multiple **scheduler** fire delivers `/compact` **alone** — the prompt resumes next fire — because `/compact`'s summarization turn is async and a paste landing mid-turn is dropped. Consequences codified after the scraper-opt incident (2026-07-05): the compact fire (1) claims **no** Consumer batch (a claimed batch would be acked without ever being delivered — silent item loss), (2) records its `last_run` **already-`Done` at fire time** (its prompt never reaches the agent, so no `report_done` can ever close it; a `Running` record would wedge a Consumer's run-active gate forever), and (3) reports `compact:true` in the trigger response, which the scheduler maps to `FireOutcome::FiredCompact` and floors the next-fire spacing at `COMPACT_SETTLE_SPACING_SECS` (10 min) so the following prompt can't land mid-summarization. Manual fires (`run_now` / MCP `trigger` / `resolve_stuck` refire) never compact. Operator break-glass for any run stranded `Running`: `continuous.force_done {task_id, seq, reason?}`.
...
**Claude account holds:** an active run whose transcript ends in either `authentication_failed` or Claude's first-person weekly/usage-limit banner persists an `account_blocked {run_seq, session_uid, detected_at, kind, detail}` marker. That marker blocks normal due-fires and dead-persistent supervision so a shared account outage cannot consume more queue batches through futile retries. Credential shape is not a recovery oracle: the scheduler requires `~/.cm/claude-probe-state.json` to record an end-to-end `OK` check newer than `detected_at`. A Consumer's prompt-delivery ack has already moved the blocked run's items to `consumed`, so recovery first reads `.queue/batch-<seq>.json` and idempotently re-enqueues each payload using its original dedupe key (or a stable original-ID-derived key); a missing/corrupt batch leaves the hold in place rather than losing work. It then atomically closes the matching run `Failed`, clears the marker/wedge streak, kills the poisoned session, and reopens scheduling after a short reap-settle floor. The seq+uid+Running guards make a concurrent completion/new fire win; the durable marker makes the hold survive daemon re-exec and `/login` transcript bookkeeping.
