...
Continuous-task orchestrators use descriptive `<task>-orchestrator` names instead of short codenames, within the same 40-grapheme limit. Agents expose self-renaming through `chat_rename`, which calls `session.set_name` without a caller-supplied UID. Obtain `expected_name_revision` from `chat_open().name.revision`; retry with the identical request ID/body/origin. A stale revision requires a fresh read and new request. The viewer resolves historical actor headers through the current participant directory, falling back to the stored name when unavailable; original event snapshots and message bodies remain immutable.
