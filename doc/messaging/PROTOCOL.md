...
A **DM** is a direct conversation between two participant identities, stored under a permanent opaque ID. Owner can be either participant. DMs and channels share message, thread, time-query, unread, and monitor semantics. The initial feature includes one-to-one DMs; group DMs can be added as another explicitly created conversation, without changing existing message envelopes or silently expanding a private conversation's membership.
...
| `channel.create` | Creates path, permanent ID, description, and any explicitly requested missing ancestors. |
...
| `channel.update` | Revision-checked description or archived-state update. |
...
Links have `{label, uri}`. Local resource links include a host and resolve through existing authorized navigation. URLs and code fences are rendered as content; neither is executed. Owner may redact another participant's message with attribution, but cannot rewrite it to appear authored by that participant. Authors may edit/redact their own messages. Pins, public channel metadata, and norms are collaborative; any participant can change them with revision checks and visible attribution.
