...
* **Channels and DMs:** creation and membership metadata are coordinated initially. Offline posting is limited to existing, locally known conversations. A first DM or new channel may therefore wait on the hub. This preserves the single conversation for an unordered participant set and avoids divergent channel paths. Group membership changes remain outside the initial scope.
...
* **Monitors and wakes:** evaluate newly ingested messages at the recipient's daemon. Persist a hit/wake intent keyed by monitor and message ID before delivery. The hub echo of a locally created event cannot create a second hit. Native Claude/Codex delivery retains the existing submission/observation uncertainty rules; deduplicating transport does not promise exactly-once agent processing.
...
Owner can have several TUIs, so replicated read state must not multiply wake ownership. The first C implementation places Owner monitors and preferences on the coordinator, so watches continue while the laptop is closed; other TUIs read its results and mirror summaries. Owner monitor mutations require connectivity. Runtime ownership transfer is not exposed in this release; any future transfer must be revision-checked while connected. There is no automatic partition failover. Session-owned monitors run on their session's host; a new session does not inherit them merely by using a similar name.
...
For this route, a centralized warm send would plausibly be on the order of a few tenths of a second plus service/storage overhead. That inference is fast enough for many agent conversations. The original wake design included a two-second debounce; the deployed native
adapters now own wake scheduling. Local-first is primarily valuable for independent local operation, responsive UI/tools, and resilience to disconnects. That initial echo probe did not measure local commit latency; the implementation
probe linked below now records local commits and durable hub receipts. Do not advertise a sub-10-ms guarantee, treat half an RTT as a measured one-way delivery time, or add a fresh SSH handshake to every message.


## Implementation and rollout

[CROSS_MACHINE.md](CROSS_MACHINE.md) documents the implemented endpoints, pairing,
coordinator handoff, Owner execution-host choice, task bindings and TUI controls.
Offline `@here` freezes the last complete membership revision known to its origin.
[Milestone C evidence](MILESTONE_C.md) separates tested code from live deployment;
[the implementation probe](latency-c-probe.json) measures durable message transport
on the real SSH route using disposable roots.
