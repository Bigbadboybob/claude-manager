...
async def main():
 case='native-appserver-remote-tui';env,work=make_env(case);mock=await Mock(case).start();config(env,work,mock.port);sock=str(ROOT/case/'app.sock');log=open(ROOT/case/'server.stderr.log','wb')
 ...
 try:
  ...
  tui=await TUI(case,env,work,args=('--remote','unix://'+sock,'resume',tid)).start()
  try:
   await asyncio.sleep(2);await tui.send('UNSENT_REMOTE_TUI_DRAFT');res['remote_tui_before']=tui.snapshot('remote-tui-before')
   pos=len(c.events);await c.call('turn/start',{'threadId':tid,'input':[{'type':'text','text':'CM_NATIVE_NOTIFICATION_WITH_DRAFT'}]});await c.wait_event('turn/completed',after=pos);await asyncio.sleep(.4)
   res['remote_tui_after']=tui.snapshot('remote-tui-after');res['remote_tui_draft_preserved']='UNSENT_REMOTE_TUI_DRAFT' in res['remote_tui_before'] and 'UNSENT_REMOTE_TUI_DRAFT' in res['remote_tui_after']
   res['draft_was_submitted']=any('UNSENT_REMOTE_TUI_DRAFT' in json.dumps((r['body'] or {}).get('input',[])) for r in mock.requests)
  finally:await tui.close()
  write_json(ROOT/case/'results.json',res);print(json.dumps({k:v for k,v in res.items() if k not in ['turn_after_original_disconnect','remote_tui_before','remote_tui_after']},indent=2),flush=True)
 finally:
