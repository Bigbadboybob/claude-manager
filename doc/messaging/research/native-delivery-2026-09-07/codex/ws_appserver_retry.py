...
async def main():
 case='native-appserver-control-retry';env,work=make_env(case);mock=await Mock(case).start();config(env,work,mock.port);sock=str(ROOT/case/'app.sock');log=open(ROOT/case/'server.stderr.log','wb')
 ...
 try:
  ...
  pos=len(b.events);before=mock.responses;retry=await b.call('turn/start',{'threadId':tid,'clientUserMessageId':'native-idle-wake','input':[{'type':'text','text':'NATIVE_IDLE_WAKE'}]});await b.wait_event('turn/completed',after=pos);res['same_client_id_retry']={'new_turn':retry['turn']['id']!=wake['turn']['id'],'response_delta':mock.responses-before}
