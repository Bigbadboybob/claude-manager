...
Until 2026-09-10 the laptop TUI refreshed the whole task list on a five-second timer (`backend_loop` fetched `GET /tasks` for the Sessions view and again for Planning on every tick); each response was 5,006,310 uncompressed bytes for 1,424 non-archived tasks (2.44 MB of prompts, 1.32 MB of descriptions), and the API ignored `Accept-Encoding: gzip`. The cost figure is deliberately conservative: ONE 5,006,310-byte response every 5 s around the clock is 80.57 GiB/day, about $278/month at the observed ~$0.115/GiB over 30 days, and gzip alone (1,630,534 B at level 5) would leave ~$91/month. The actual observed laptop rate was five full downloads in a 30-second sample, so the real per-viewer figure was of that order; sending only what changed removes nearly all of it. The full cost audit is in the predictionTrading repo, `agent_docs/cost-audit-2026-09-10.md` (branch `cm/lower-cost`); this is an illustrative per-viewer figure, not an attribution of the whole CM network bill ($15.33/day baseline, $18.66/day Sep 3–7 across all consumers).
...
* No `since`/`epoch`, an `epoch` from another log lineage, a cursor below `pruned_through` (retention) or above the sequence: reply `{"reset": true, "epoch", "cursor", "tasks": [...full list...]}`. The snapshot reads its cursor **before** the rows in one transaction, so every change with seq ≤ cursor is reflected (rows may be newer, which later change rows re-deliver idempotently).
...
* **trading portal** (`portal/backend/services/backtest_sync.py` in predictionTrading): every 20 s it downloads `GET /tasks?project=predictionTrading&include_archived=true` and then `GET /tasks/{id}/artifacts` for every backtest task in running/done/blocked/archived (≈ 360 artifact requests + 1 list per 30 s observed). The cost audit captured this traffic going to the portal's public IP (34.11.48.87) — the two projects have separate, overlapping VPCs — so it is billable egress too, not internal; it gains gzip automatically. It could move to `/tasks/changes?project=predictionTrading&include_archived=true`, but NOTE: `db.add_task_artifact` only INSERTs into `task_artifacts` and does not touch the task row, so the task feed does not signal new artifacts; replacing artifact polling needs artifact events (or an artifact-invalidation trigger) first. Tracked as predictionTrading task 0c2402cf.
...
* **Epoch on restore.** `task_change_meta.epoch` is minted once (`gen_random_uuid()` default when the row is first inserted) and a database restore brings the row back with the SAME epoch. A restore to an earlier point therefore does NOT rotate the epoch by itself: a client holding a cursor above the restored sequence is caught by the `since > last_seq` check, but a cursor that is numerically still valid against the restored log would resume against different history. After any restore or manual edit of `task_changes`, rotate the epoch explicitly: `UPDATE task_change_meta SET epoch = gen_random_uuid()::text;` — every client then resnapshots on its next poll.
* **Lock order.** Every application transaction that writes `tasks` or `initiatives` after taking a row lock must call `db._serialize_task_writes(conn)` FIRST (it takes the same advisory lock the trigger takes). The trigger's `BEFORE STATEMENT` lock only precedes the locks of the statement it fires for, not locks an earlier statement in the same transaction already holds: `update_initiative` used to `SELECT ... FOR UPDATE` the initiative and then update tasks, so a concurrent single-statement initiative update (advisory lock held, waiting on that row) and the multi-statement transaction (row held, waiting on the advisory lock) deadlocked. `tests/test_task_changes.py::test_row_lock_before_task_write_deadlocks_without_serialize` reproduces the hazard and `..._serialized_transaction_does_not_deadlock` pins the fix. A foreign client that row-locks and then writes in one transaction without the advisory lock can still hit PostgreSQL's deadlock detector (one side is aborted after `deadlock_timeout`, 1 s); nothing is lost, the aborted side retries.
