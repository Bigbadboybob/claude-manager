...
`DETECTIVE_WORKER_PROMPT.md` is a worker brief, not an orchestrator `default_prompt`. Audit it separately. The six new periodic-task sources capture their live prompts; the new scraper-opt source preserves its live body without overwriting a historically different template. The source branch was integrated and pushed to PredictionTrading main at `08067c77f`; all ten live prompts now match it. For a future change, apply each live prompt only while its task is paused, and make required worker briefs available in the actual consumer worktrees before their cutovers. Verify live content by re-reading it, not just from the update response.
