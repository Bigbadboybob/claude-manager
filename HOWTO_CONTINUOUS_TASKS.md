...
- **Handoff transport.** The daemon creates `ct/<slug>` at `continuous.create` and keeps the orchestrator and every attributable worker joined. Workers post in it mentioning the orchestrator; the orchestrator mentions workers; a mention wakes the addressed session natively without waiting for the next scheduled scan. Verify one round trip independently of cadence (§6).
...
## 3. Write the standing instructions and the dispatch (the heart of it)

The prompt IS the orchestrator, and it comes in two parts ([continuous-standing-instructions.md](doc/continuous-standing-instructions.md)): `standing_instructions` holds everything durable (lane, GATE, lifecycle, the cycle procedure, the channel contract; 15–40 KB) and the daemon writes it into the worktree as the engine's own project-instruction file (`AGENTS.override.md` for Codex, `CLAUDE.local.md` for Claude), which the engine loads into its prompt prefix and re-injects after every compaction. `default_prompt` is only the per-fire dispatch (~0.6 KB): run one cycle now, where the standing instructions are, close with `report_done`. Never paste the procedure into the dispatch. Copy a live task's standing text as scaffolding:

```bash
python3 -c "import json;d=json.load(open('/home/lucas/.cm/continuous-tasks/scraper-triage/state.json'));print(d['standing_instructions']);print('--- dispatch ---');print(d['default_prompt'])"
```

Keep these **proven idioms** (every live orchestrator uses them):

- **Orchestrator identity is scheduler-owned.** The daemon names the bound session `<task_id>-orchestrator` on every fire and releases the name from the previous instance; the prompt does not need to claim it (and workers cannot: the suffix is reserved). Tell workers to pick a short codename, ideally the subtask id. Include `~/.cm/policies/continuous-review-routing.md` in every prompt and worker brief and keep that shared policy deployed on the execution host.

- **Review routing and lifecycle (Owner, 2026-09-14).** Include [continuous-review-routing.md](doc/continuous-review-routing.md) in every new orchestrator and worker brief. Routine worker handoffs are task-channel posts mentioning the parent (`chat_open().continuous.orchestrator`); the parent wakes on every channel post and answers on that wake. Drain and acknowledge the inbox before cadence gates, review only the handed-off work, and deduplicate by task ID + artifact SHA + stage. A mention does not admit a new scan or queue batch. Promote to `owner_review` only when the policy's ready-for-approval list holds, with one approval packet in the channel. Keep schedules, queue admission, completion monitors and periodic reconciliation. Only the orchestrator escalates a reviewed decision that actually requires Owner. Maintain `metadata.continuous_stage`, actual UTC `stage_updated_at` and `next_action`; distinguish `review_queued`/`reviewing` from `owner_review`. Preserve visible live sessions while tasks remain unfinished. Activity/idle state is never a review decision.
...
- **Recover unfinished tasks using the SAME task/worktree** — `mcp_start_session(task_id=<existing subtask id>, …)`, not another `create_subtask`. A relaunched worker is joined to the task channel automatically; the channel history is its context. Workers resolve the current parent through `chat_open().continuous` after a replacement. Preserve pause state and concrete infrastructure holds; record failed recovery rather than repeatedly launching a known-broken worker. Already terminal work is reviewed for cleanup, not relaunched merely because its session exited.
...
rpc("continuous.create", {
    ...
    "standing_instructions": standing,  # from §3 — the durable procedure (file-delivered)
    "default_prompt": dispatch,          # from §3 — the ~0.6 KB per-fire dispatch
    ...
    "task_channel": "auto",              # auto (default): daemon creates + binds ct/<slug>;
                                         # manual: pass "messaging": {"channel_id": "<uuid>"};
                                         # off: no task channel
})
# -> {created, task_id, workspace_id: "ws-<slug>", worktree_path: ".../<repo>-<slug>",
#     channel: {channel_id, path: "ct/<slug>", created, deferred: []}}
```

`channel.deferred` is non-empty when the messaging coordinator was unreachable at create time; the daemon's refresh loop finishes the job within seconds, and `continuous.ensure_channel {task_id}` forces it (idempotent — also the repair entry point after a channel was deleted by hand).

Other accepted params (see `ContinuousCreateParams` in `daemon/src/control/methods.rs`): `slug`, `workspace_id`, `project`, `start_branch`, `skill`, `modes`, `max_runtime_secs`, `downstream`, `enqueue_to`, `retention`, `messaging`. To change any of these later on a **live** task, use `continuous.update` (preserves `run_count` + history) — see §8.
...
Also verify one actual channel round trip: `continuous.list` shows `messaging.path = ct/<slug>` with the current session as `orchestrator` and `orchestrator_name = <task_id>-orchestrator`; a worker's `chat_open().continuous` names that orchestrator; the worker posts in the channel mentioning it; the parent's native wake is observed (`notification_status`), its inbox is read/acknowledged, the artifact reviewed and the stage persisted; the parent's reply mentioning the worker wakes the worker. No routine Owner notification should occur. A prompt update, idle parent or successful `chat_send` alone does not prove this path. Do not restart a busy parent just to pass the smoke test.
...
- **Health read:** `continuous.list` → per-task `run_count`, `schedule`, `next_fire_at`, `current_session_uid`, `last_outcome`, `in_flight`, `account_blocked`, `review_kind`, `planning_task_id`, and `messaging` (`channel_id`, `path`, `members`, `orchestrator`, `orchestrator_name`, `revision`). Or read `~/.cm/continuous-tasks/<task>/state.json` directly.
- **Review delivery health:** read the task channel (`chat_read(channel="ct/<slug>")`) — unanswered worker mentions and system `held` notices are the loop's backlog — and inspect the current parent's native `notification_status` separately from scheduled-run health. A completed scheduler run does not prove worker messages were processed. `continuous.ensure_channel {task_id}` repairs a missing channel/binding/name. See [native notifications](doc/messaging/NATIVE_NOTIFICATIONS.md).
- **Change a live task in place:** `continuous.update {task_id, <field>}` — **preserves `run_count` + run history** (no delete+recreate). Common: steer `standing_instructions` (the file is rewritten immediately; a live thread picks it up on its next fresh thread or first post-compaction turn), set `compact_every`, change `schedule`, backfill `review_kind`. Applied fields take effect next fire; the live session keeps running. Migrate a legacy single-prompt task with `scripts/migrate_standing_instructions.py --host <host>`.
- **Review its output:** `/triage-review <task>` (or no-arg to enumerate reviewable tasks via `review_kind`). Fix-first tasks → walk the merge queue; investigate-first → read the proposals + decide. Post every disposition (landed SHA, deploy time/target, terminal state, or returned-for-correction) into `ct/<slug>` mentioning the orchestrator. The orchestrator NEVER merges triage fixes itself — you do — **unless** the task is explicitly designed to auto-merge high-confidence fixes (then it gates on build+test green and pushes only `main`).
