...
- **Scope addition (phase 3):** daemon-side codex rollout tracking — `session_watch`/`transcript_detect` tail the session's `~/.codex/sessions/YYYY/MM/DD/` rollout files and re-stamp the resume key on rotation, the codex analogue of the claude Stop-hook re-stamp.
- Until that lands, the decision table's "resume from pinned stamps" rows carry an explicit caveat: a codex session that compacted since spawn resumes pre-compact history. Documented limitation, not silent behavior.
...
| Re-exec machinery itself / incompatible state schema | drain (H3) → legacy restart | killed deliberately, resumed from H1-pinned stamps (codex: pre-compact until rollout tracking lands) |
...
4. **Full rehydrate**: escrow/commit-gate transaction, complete state-inventory table (fail review on "unclassified"), pidfd handoff + `waitid(P_PIDFD)`, watcher re-adoption from checkpoints, workflow/continuous rebuild, manifest.watch generation bump, `--verify-handoff` + schema version + frozen-snapshot revalidation, rollback-FD exec + attempt state machine + deliberate-kill terminal fallback, codex rollout tracking. Sandbox matrix: idle claude session, mid-turn session (background command writing), bash session, workflow participant, session that dies mid-handoff, corrupt manifest, rollback exec.
5. **TUI local auto-reattach**: extend the remote transport-EOF → `pending_remote_reattach` flow to `local` host rows; handle the offset/generation reset (attach as a new stream); delete cm-redeploy's TUI kill.
6. **`daemon.restart` RPC + cm-redeploy switch**: build → verify-handoff → RPC → poll health `build_id`; keep `--hard` as the legacy path.
...
- **Codex lineage staleness on hard paths** — until rollout tracking lands (phase 4), a compacted codex session resumes pre-compact history on any hard restart; documented in the decision table.
