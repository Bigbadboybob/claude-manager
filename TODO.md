...
### P2 — Correct continuous `report_done` audit run mode (implemented locally; deploy pending, 2026-09-06)
...
The local fix emits the completed task's actual mode, with coverage for fresh/persistent completions, run identity and no audit entry for a completion no-op. Historical logs stay intact; the [Codex migration manifest](doc/continuous-codex-migration.md) should annotate this known discrepancy and derive mode from the definition plus matching fire record until the fix ships. No completed work should be rerun or interrupted for this audit correction.
