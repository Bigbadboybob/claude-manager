...
## Current stage and activity indicators

The current lifecycle contract is [continuous-review-routing.md](doc/continuous-review-routing.md). Subtask label text carries the stage color, with a fixed legend below the list; focused selection overrides the stage color. `review_queued`, `reviewing` and `owner_review` distinguish waiting for parent review, active parent review and a concrete Owner decision. Activity glyphs describe the session independently. Human-oriented guide: HOWTO_CONTINUOUS_TASKS.md → "Continuous-column stage and activity indicators". The original layout plan below remains historical where later revisions supersede it.

- **spinner (green)** — session Running.
- **`●` white** — legacy planning-status attention signal for raw `blocked`. Use the durable stage and next action to confirm an actual Owner decision. Routine parent reviews and monitoring stay planning `running`.
...
- **`◇` dim** — idle activity; stage and next action identify who acts next. A worker's task-channel mention wakes the parent for review before its next scheduled scan. Idle does not mean approved.
