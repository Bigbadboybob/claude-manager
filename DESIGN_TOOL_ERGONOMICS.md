...
**Status:** P0 ✅ + P1 (isolation flag + bash advisory) ✅ + P2 (field trim + tool-sprawl docstrings) ✅ + teardown-safety guard ✅ IMPLEMENTED (branch `cm/improve-cm-ergonmoics`, 2026-07-06). The MCP-layer work is `mcp_server/` (Python); the teardown-safety fix is Rust (`daemon/` + `tui/`). Runs locally, no deploy. **Remaining items reviewed 2026-07-06 (see per-section notes):** push-instead-of-poll is already achieved via the `cm-wait` background CLI (close it); the "auto-clean teardown" turned out to be a real force-remove data-loss bug and is now fixed; bash sentinel-capture is defer/drop (needs a daemon terminal buffer, low leverage); P2 envelope-*uniformity* is a deliberate no-op (breaking change, cosmetic). Nothing substantive is left open. Written 2026-07-06 from a dogfooding pass: an agent (this session) exercised the read-only cm tools live plus one full write round-trip (`start_session` → `send_input_and_wait` → `read_last_turn` → `kill_session` on a throwaway bash session), and compared the experience against the Claude Code `Agent` / `Task*` / `SendMessage` / `Monitor` tools that ship in the harness today.
...
>
> **Teardown safety — ✅ IMPLEMENTED (Rust, 2026-07-06), and it turned out to be a bug, not a nicety.** Reviewing the teardown side surfaced that `mark_subtask_done(close_worktree=true)` calls `remove_worktree()`, which runs `git worktree remove --force` with NO dirty-check (`daemon/src/worktree.rs`, `daemon/src/control/methods.rs`, `tui/src/control/methods.rs`) — so uncommitted/untracked work in a subtask worktree was silently destroyed. Agent's "auto-clean if unchanged" is the *inverse* (it keeps changed worktrees). Fix: a new `worktree::worktree_is_dirty()` (non-empty `git status --porcelain`; a missing dir counts as clean so the idempotent remove still works) is checked in BOTH `mark_subtask_done` handlers *before* any sessions are closed. A dirty branch worktree now returns a `Conflict` error and tears down nothing (sessions stay alive so you can go merge); committed work is always safe on the branch ref, so only the working tree is guarded. A new `force=true` flag on the tool overrides for the "yes, discard it" case. `remove_worktree`'s signature is untouched, so user-initiated `A-x` delete (which legitimately force-removes) is unaffected. Test: `worktree_is_dirty_tracks_working_tree_state`.
...
> **Reassessed on review — this is largely already solved, and I over-scoped it above.** `mcp_server/wait.py` (the `cm-wait` CLI) IS the push path: run it under `Bash(run_in_background=true)` and the Claude Code harness wakes the orchestrator when the background process exits with the JSON result — exactly the `<task-notification>` re-invocation, achieved through the harness's own background-task mechanism, no new cm infra. The orchestrator stays free while workers run instead of parking in a blocking MCP call. The only real gap is narrow: `cm-wait` wraps the fan-out wait, and there's no single "spawn a worker and background-wait on it" convenience — but that's already composable (`start_session(wait=false)` → `cm-wait <uid>` in background) and is Python polish, not the daemon lift the original text implied. **Verdict: close it.** A daemon→agent push channel would only matter if we wanted to drop `cm-wait`'s background-bash indirection, which isn't worth it.
