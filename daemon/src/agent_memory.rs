...
pub fn validate_transcript_id(id: &str) -> Result<()> {
    if id.is_empty() {
...
pub fn save(spec: SaveSpec<'_>) -> Result<Snapshot> {
    ...
    let root = root_dir().ok_or_else(|| {
        SnapshotError::Io(io::Error::new(io::ErrorKind::NotFound, "HOME is not set"))
    })?;
...
fn save_into_tmp_dir(spec: SaveSpec<'_>, tmp_dir: &Path, final_dir: &Path) -> Result<Snapshot> {
...
pub(crate) fn rewrite_codex_line1(line: &str, new_id: &str, new_cwd: &str) -> Result<String> {
...
pub fn claude_memory_dir(cwd: &Path) -> Option<PathBuf> {
    let home = std::env::var_os("HOME").map(PathBuf::from)?;
    let path_str = cwd.to_str()?;
    let encoded = path_str.replace('/', "-").replace('.', "-");
    Some(
...
mod tests {
    use super::*;
    use crate::test_support::env_lock as home_lock;
    ...
    fn ymd_from_unix_secs_known_dates() {
        ...
        assert_eq!(ymd_from_unix_secs(47_541 * 86_400 + 86_399), (2100, 3, 1));
    }
    ...
    fn load_accepts_well_formed_manifest() {
        ...
        assert_eq!(
            s.manifest.source_transcript_id,
            "019e3bd4-b2c6-7ac2-ba95-cf8d98fc2236"
        );
    }
    ...
    fn clone_claude_failure_leaves_preexisting_memory_files_untouched() {
        ...
        let result = clone_claude_inner(&src_transcript, Some(&src_mem), &dst_transcript, &dst_mem);
    ...
    fn clone_claude_memory_commit_failure_leaves_transcript_uncommitted() {
        ...
        assert!(
            result.is_err(),
            "expected memory rename failure, got {result:?}"
        );
        assert!(
            !dst_transcript.exists(),
    ...
    fn save_rejects_empty_source_transcript() {
        ...
        if snap_root.exists() {
            for entry in fs::read_dir(&snap_root).unwrap() {
                ...
                assert!(
                    name.starts_with(".tmp-") || name == "snap" && false,
    ...
    fn codex_clone_only_touches_line_one() {
        ...
        let first_trim = first.strip_suffix('\n').unwrap();
        let rewritten_first = rewrite_codex_line1(first_trim, "new-id", "/new/cwd").unwrap();
        ...
        let v1: serde_json::Value = serde_json::from_str(out.lines().next().unwrap()).unwrap();
        assert_eq!(v1["payload"]["id"], "new-id");
        assert_eq!(v1["payload"]["cwd"], "/new/cwd");
        assert_eq!(v1["payload"]["misc"], 42);

        let original_rest: String = original.split_inclusive('\n').skip(1).collect();
        let out_rest: String = out.split_inclusive('\n').skip(1).collect();
        assert_eq!(out_rest, original_rest);
    }
}

pub fn read_transcript_head_tail(snapshot_dir: &Path, n: usize) -> (Vec<String>, Vec<String>) {
...
/// Host-local catalog operations, also used directly by the local TUI.
pub fn catalog(params: &serde_json::Value) -> Result<serde_json::Value> {
    use serde_json::json;
    let field = |key: &str| {
        params
            .get(key)
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| SnapshotError::InvalidName(format!("missing {key}")))
    };
    Ok(match field("action")? {
        "list" => serde_json::to_value(list()?)?,
        "load" => serde_json::to_value(load(field("name")?)?)?,
        "rename" => {
            rename(field("name")?, field("new_name")?)?;
            json!(null)
        }
        "delete" => {
            delete(field("name")?)?;
        ...
        "preview" => {
            let snap = load(field("name")?)?;
            serde_json::to_value(read_transcript_head_tail(&snap.dir, 5))?
        }
        other => {
            return Err(SnapshotError::InvalidName(format!(
                "unknown catalog action {other}"
            )))
        }
    })
}
