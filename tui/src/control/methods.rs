...
struct StartWorkflowParams {
    ...
    #[serde(default)]
    goal: Option<String>,
    /// Optional map of role name → existing `session_uid`. Each
    /// referenced session must live in the target task's workspace
    /// and be in the caller's auth scope. Roles not present are
    /// spawned fresh as usual.
    #[serde(default)]
...
pub fn start_workflow(app: &mut App, caller_uid: &str, params: &Value) -> MethodResult {
    ...
    // Resolve role_sessions uids → session indices within target_wi.
    // Each must live in the target workspace (cross-workspace binding
    // makes no sense for workflows) and pass the caller's auth check.
    let mut existing_role_sessions: std::collections::BTreeMap<String, usize> =
        std::collections::BTreeMap::new();
    if let Some(rs) = p.role_sessions.as_ref() {
        for (role, uid) in rs {
            let (wi, si) = find_live_session(&app.workspaces, uid).ok_or((
                ErrorCode::NotFound,
                format!("role_sessions[{}]: session_uid '{}' not found", role, uid),
            ))?;
            if wi != target_wi {
                return Err((
                    ErrorCode::InvalidParams,
                    format!(
                        "role_sessions[{}]: session '{}' is not in the target task's workspace",
                        role, uid
                    ),
                ));
            }
            ...
            if !caller_authorized_for(&caller, &app.tasks, wi, target_task.as_deref()) {
                return Err((
                    ErrorCode::Unauthorized,
                    format!(
                        "role_sessions[{}]: session '{}' is outside caller's scope",
                        role, uid
                    ),
                ));
            }
            existing_role_sessions.insert(role.clone(), si);
        }
    }
