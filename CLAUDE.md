...
- `A-s` — add a session to the focused task. The form has a **Resume** field (Tab to it, Enter opens a picker of the worktree's claude/codex transcripts, newest first with age, size, and the first prompt): the new session spawns `claude --resume <id>` / `codex resume <id>` with its row bound to that transcript from the start. This is the native replacement for "A-s, then `/resume` inside the pane". Mutually exclusive with the Seed field.
- `A-a` — attach
- `A-w` — close session
