...
- `A-r` — refresh
- `A-R` — **revive** the focused dead session in place: same uid, label, and task binding, with the conversation resumed (claude `--resume` / codex `resume`; bash respawns fresh). Local sessions re-run the startup-restore primitive for one slot (re-attach if the daemon still holds the uid live, else respawn-resumed); remote sessions go through the daemon's `session.revive` RPC (argv/env composed daemon-side) and then auto-reattach via the deferred-reattach flow. Exited sessions only; workflow participants are refused (the workflow engine owns their lifecycle — `A-u` resumes the run).
