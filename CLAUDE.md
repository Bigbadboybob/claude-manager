...
Channel creators start as admins; Owner always retains admin access. Admins can add a known agent with `chat_channels(action="add_member", path="...", participant_id="<id from chat_people>", request_id="...")`; added members can leave, and retrying an old add does not rejoin them. Owner uses `u`, then `Ctrl+a`, to search and add a member. Admins manage display names, descriptions, pins, and access policy, including appointing other admins or enabling editing for all agents. Paths/IDs stay stable, and message deletion is not available. Owner uses `n` to create a channel, `S` for settings, `p` to pin/unpin the selected message, and `P` to toggle pinned messages. See [channel controls](doc/messaging/CHANNELS_AND_PINS.md).
...
- **`daemon/`** — `cm-daemon`, the persistent host daemon. Owns session PTY lifecycle, manifest, manifest.watch broadcasts, workflow state, and the control-socket dispatcher. The TUI launches it automatically at startup if it isn't already running; mandatory since slice 10f (Phase 1's default flip).
  ...
  **Restarting the daemon** (decision table, DESIGN_HOLDER_BRAIN_SPLIT § Operator decision table — pick the weakest primitive that covers the change):
  ...
  | Holder binary (rare) | `daemon.upgrade_holder {holder_path}` — S15-gated; the holder re-execs itself; verify `holder_build_id` changed while `holder_epoch` + session count did not | untouched |
  | Leave the split | delete `~/.cm/holder-binary` FIRST, then `daemon.split_rollback {monolith_path}` | untouched (ride back into the monolith at the same PID) |
  | Holder/manifest machinery itself / a manifest-schema bump | `scripts/cm-redeploy --hard` (drain → legacy kill+restart) | killed deliberately, resumed from spawn-pinned stamps (codex: at most one ~5s rollout-observation interval stale) |
  | Machine reboot | startup-restore (the flip file relaunches the split) | die, resume from pinned stamps (same codex caveat) |

  `daemon.restart` / `rollback_brain` / `upgrade_holder` / `migrate_split` / `split_rollback` are strong-operator gated (fail CLOSED when the daemon started without `CM_OPERATOR_TOKEN` — the TUI sets it and persists the token at `~/.cm/operator-token`, which cm-redeploy reads); `daemon.restart` still refuses on `[tls]`-configured daemons (listener handoff unimplemented — use the `--hard` row there). Verification is fire-and-verify: the RPC connection dies at the exec/brain-exit, so callers poll `daemon.health` — in split mode the surface is `split`, `holder_epoch` (the brain-generation counter), `holder_build_id`, `brain_pid`, `breaker_state`, `brain_restarts`, `sessions`; the all-clear after a deploy is `breaker_state == "running"` with no `brain_restarts` delta through the breaker's 10-minute stability horizon.
...
- **`mcp_server/`** — MCP server exposing tools that agents running inside the TUI can call:
  ...
  - `ping`, `list_sessions`, `list_sessions_grouped`, `mcp_start_session`, `send_input`, `read_session_output`, `kill_session` — agent orchestration. Routed per-method by the resolver: session/PTY-touching methods go to `~/.cm/daemon.sock` (the daemon owns the registry); workflow-touching methods stay on `~/.cm/tui.sock`. Auth keys off `CM_TUI_SESSION_ID` injected when the TUI spawns the agent; descendant-only scope by default. `ping` now reports the caller's own `global_perms` / `task_id` / `workspace_id`; `list_sessions` entries carry `task_id` / `workspace_id` / `worktree_path` / `global_perms` (TUI-owned rows included — the TUI's session-snapshot push carries workspace + checkout, so a TUI-launched sibling is no longer a workspace-less row) plus `workspace_shared_with` = `[{session_uid, label}]` on live rows sharing a `worktree_path` with another caller-visible session (absent when the checkout is unshared; computed MCP-side from the already-authorized rows, never a wider re-query), and `list_sessions_grouped` returns them as a workspace→task tree with the same hint. `mcp_start_session` **mints a worktree for a workspace-less task** (`start_session(task_id=X)` where nothing has ever run for X — the `propose_task` shape): a `cm-sub/<slug>-<first 7 of the task id>` branch cut from the project's trunk (`main`/`origin/main`/`master`/`origin/master`, local-first), registered as a workspace and bound to X, so the worker runs in a clean tree and later spawns on X join it. The suffix is task-id-derived, so a re-mint is idempotent; the workspace is registered only AFTER the checkout exists, so a failed mint leaves nothing half-made. Shared implementation for both routes: `daemon/src/worktree.rs::mint_task_worktree`. **A bound task whose worktree was reaped self-heals**: both routes run `worktree::ensure_worktree_materialized` on the target before spawning — present → spawn; missing → `git worktree add` from the task's branch (resolved from git's prunable registration, then the CM dir-name scheme, then the row's `wip_branch`, preferring a candidate with commits beyond trunk) + the same `setup_worktree` provisioning a first-time subtask gets (incl. a `.venv` → `<main_repo>/.venv` symlink fallback), reported as `worktree_recreated`; unrecoverable → the call FAILS naming the path and reason. A zero-commit branch (the decoy signature) is surfaced in `worktree_warning` with any same-slug sibling that has work, and a re-mint re-attaches an existing `wip_branch`/mint branch instead of cutting a decoy. Backstop: the spawn core refuses any `working_dir` that isn't a directory. Pre-fix `start_session(task_id=<reaped subtask>)` returned success with an absent `worktree_path` (the PTY forks before the child's chdir fails) and the agent sat `awaiting_input` with an empty transcript. `allow_shared_workspace=true` opts back into the old co-tenant behavior (spawn in the CALLER's checkout) and reports it loudly — `shared_workspace` / `warning` / `workspace_shared_with` in the return; when a mint is impossible and the flag wasn't passed the spawn is REFUSED, with the flag named in the error. Pre-fix the daemon route silently co-tenanted (three `propose_task` workers rewrote HEAD under their proposer's live session) while the TUI twin answered `NotFound` — same call, two answers. `mcp_start_session` also returns `worktree_path` (where the child actually landed — a branch-mode subtask's `task_id` puts it in ITS worktree, not the caller's), `task_id` when the child is bound, and `prompt_source` (`caller` / `task` / `none`): an agent spawn that names a task EXPLICITLY (`task_id` arg or `isolated=true`) with an empty/absent `prompt` auto-delivers that task's stored description+prompt — best-effort (a missing prompt or unreachable planning API just spawns promptless), never for `bash` (whose "prompt" is a command line the shell would execute), and never off a merely inherited binding (a promptless spawn without `task_id` stays promptless for the spawn-then-`send_input` pattern). `kill_session` records provenance (`killed` / `killed_by` / `exited_at` on the exit tombstone), surfaced on exited `list_sessions` rows, in `read_last_turn` / `read_session_output`, and in monitor fire messages; `killed_by` is a who-OR-what rendered verbatim — a session uid, `"operator"` for the TUI/watchdog routes, `"memory-cap"` for a cgroup/OOM cap kill (which used to reach the tombstone as a plain exit, so its truncated tail read as a final report), or `"<uid> (mark_subtask_done)"` for the sweep that closes a done subtask's sessions. Killing an already-gone session answers `not_found` with "session '<uid>' already exited at `<ts>`" instead of a bad-uid-shaped message.
  - `report_done(reason?)` — the caller's own "my work is finished" signal, and the termination predicate `until="final"` monitors need. Every Session caller gets the session-scoped mark: `list_sessions` / `read_last_turn` / `read_session_output` report `status="reported"` (plus `reported_done` / `reported_done_at` / `report_reason`, carried onto the exit tombstone) instead of the ambiguous `awaiting_input`, which conflates a final report, a pause mid-task, and a turn that ended with background work still running. A continuous-task tick ALSO gets the run flip (`Running → Done`) it always did — `done` / `task_id` in the return still describe THAT and are `false` / `null` for everyone else, so the shape is wire-compatible. The mark is superseded by new input (derived from `last_input_at`, so no input path can forget to clear it), so a worker handed follow-up work reports again when that is done. Widened rather than split into a second RPC: one fact, two consumers, and two verbs would fail silently on a wrong guess.
  - `monitor_sessions`, `list_monitors`, `cancel_monitor` — **async waits** (the non-blocking replacement for parking in `wait_*` calls). A monitor watches sessions in the background (MCP-server-resident) and, on completion, delivers a `[cm-monitor <id>]` message into the CALLER's own session — waking it through a native automated event. `start_session` (wait=false + a prompt to run — an auto-delivered task prompt counts) and `send_input` auto-register one per worker (`notify_on_done=false` opts out), so orchestrators should spawn/prompt, END THEIR TURN, and let the fire wake them. Delivery uses a shared durable native queue: Claude's own-child session socket and a CM-owned Codex app-server. No new notification types into the PTY or uses the Stop-hook inbox. `notification_status` exposes connection health and pending/submitted/observed/uncertain receipts; an ambiguous submission never earns an automatic resend. The Stop hook still reports turn state and drains old inbox files during rollout. New Codex sessions use the owned backend; existing embedded sessions need a deliberate restart/resume, while compatible Claude sessions can reconnect MCP. See [native notifications](doc/messaging/NATIVE_NOTIFICATIONS.md). Monitors are **edge-triggered by default**: they arm against the watched session's transcript at registration and fire only on a NEW completed turn (or exit), so re-arming on an already-idle session doesn't instant-fire its stale reply — registration returns `already_idle` for sessions that are at their prompt right now (`edge=false` restores level-triggering). Two independent axes: `mode` = how many must finish (`any` / `all`), `until` = what finished MEANS per session — `turn_end` (default) or **`final`**, which completes only on an exit or an explicit `report_done` and silently RE-ARMS past every interim turn end (counted onto the entry as `interim_turn_ends`). Final mode is the one-watch-one-wake-up answer to a worker that ends its launch turn with background subagents still running; `mode="final"` is sugar for `mode="any", until="final"`, `until="task_done"` a synonym, and `start_session` / `send_input` take `notify_until` for the monitor they auto-register. A stale report present at arm time is anchored (returned as `already_reported`, never fired), and a session that can't report — any `bash` one — completes a final watch only by exiting, so pair it with a generous `timeout_s`. `cancel_monitor` is terminal from any state (stops the watch and retracts pending native notices; already-submitted events cannot be recalled) and accepts `monitor_id="all"` as the one-call off switch. Fire messages say HOW each session finished: a killed one reads `- <uid> (killed by <who> at <ts>)` with any transcript text labelled a pre-kill fragment instead of quoted as its reply, a transcript-shape idle notes the PTY is still active (background work may be running), and an idle-but-alive session is flagged as possibly an interim turn rather than completion — a session that called `report_done` reads `- <uid> (reported): …` with its reason and drops that caveat. The hook also self-reports `session.turn_ended` → `semantic_idle` in `resolve_authorized_session`, and the wait/monitor loops treat a transcript ending in a completed assistant turn as idle even when a background task's spinner keeps the PTY noisy. See NOTES.md on branch `cm/proper-cm-subagent-wait`.
  - **Global permissions**: a session may carry a `global_perms` flag that short-circuits the descendant-only scope — it can list/prompt/read/kill/spawn against ANY session. Granted by the operator (A-e session-settings toggle → `session.set_global_perms` RPC) or propagated by an already-global agent via `start_session(global_perms=true)` (escalation-guarded in `mcp_start_session`: honored only if the caller is itself global). The flag lives on `DaemonSession`/`TerminalSession`/`ManifestEntry`; auth short-circuits in `daemon/src/control/auth.rs::check_session_caller` and its TUI mirror `caller_authorized_for`. The daemon resolves it per request from its live registry (no cache), so a grant takes effect on the session's very next call — but only if it REACHES the daemon: the A-e save pushes unconditionally (not gated on the TUI's cached flag), the daemon persists + broadcasts the flip, and every respawn-at-same-uid (startup restore, `A-R` revive/forced restart, `spawn_managed_session`) carries the grant at spawn via `start_session(global_perms=…)`. Pre-fix an `A-R` respawn silently re-registered a global session ungranted while the sidebar kept showing it granted (2026-08-21). See AGENT_ORCHESTRATION.md → "Global permissions".
  ...
  - `create_subtask`, `list_subtasks`, `mark_subtask_done` — subtasks fork off a parent task with `parent_task_id` set and persist the same authenticated `metadata.filer` provenance as top-level proposals. `worktree_mode="branch"` creates a new worktree under `cm-sub/<slug-chain>-<short_id>`, cut from the parent's wip branch unless `base=<committish>` (branch mode only; sha/tag/local branch/`origin/<ref>`, resolved locally then via one `git fetch origin <base>`) names an explicit cut point — the base is resolved BEFORE the task row is created, so an unresolvable ref is a clean `invalid_params` rather than a half-made subtask. The return is `{task_id, worktree_path, base_sha, launched: false}`: `base_sha` is the commit the new checkout sits on, and `launched` is always false because nothing runs until `start_session(task_id=…)`. The TUI-served create path registers the subtask with its host daemon SYNCHRONOUSLY (`task.register_agent_subtask` — auth edge + task→workspace binding + worktree path, the same state the daemon's own `create_subtask` seeds inline) before returning. The old fire-and-forget `push_task_tree_to_daemon` was a race `start_session(isolated=true)` always lost — it calls `create_subtask` on the TUI socket and `mcp_start_session` on the daemon socket inside one MCP call, so every isolated spawn bounced `unauthorized: task '<id>' is not the caller's task or a descendant` and the launcher looked broken (retrying seconds later worked). Recording it as an `agent_task_edges` overlay also survives the reconcile that drops a just-minted subtask from the pushed snapshot.
  - `submit_backtest`, `get_backtest_result` — cloud auto-backtest: submit a predictionTrading backtest (branch + script + config) as a `kind='backtest'` task; the dispatch daemon's backtest lane runs it on an ephemeral GCP **spot** worker in `prediction-market-scalper`/`us-east4-a` (same VPC/zone as the replica + results bucket), the worker publishes bulk results to `gs://prediction-market-scalper-datasets/backtests/<run_key>/` and POSTs a compact summary to the new `task_artifacts` table (`POST/GET /tasks/{id}/artifacts`), and `get_backtest_result(task_id, wait=...)` reads it back. Headless-capable via daemon proxy methods `backtest.submit`/`backtest.result`. Agent-filed runs stamp versioned `metadata.filer` provenance from the authenticated caller: engine (`codex`/`claude`), session, parent task, workspace/worktree, physical machine, continuous-task and workflow context, managing session, and submission surface. Planning detail and the Sessions-view A-i backtest peek render the same filer fields; legacy rows simply omit them. `script="__cm_stub__"` runs an infra smoke test with fabricated results. Caveat: the `project`→repo_url default resolves from planning rows and can be wrong — pass `repo_url` explicitly. **Board hygiene**: terminal backtest rows auto-archive — the dispatch daemon flips `done` rows to `archived` 45 min after completion and `blocked` (failed/reaped) rows after 6 h (`CM_BACKTEST_ARCHIVE_DONE_SECS` / `CM_BACKTEST_ARCHIVE_BLOCKED_SECS`; ≤0 disables), but only once a result artifact exists and `worker_vm` is cleared — a terminal row with NO artifact stays on the board as an operator signal, and archived results remain fully readable by id (`get_backtest_result` already treats `archived` as terminal; A-V shows the rows). **Sessions-view rendering**: backtest tasks are never modelled as workspaces (they run entirely on the worker VM — an empty top-level sidebar row per run was pure noise, K×N of them for a perf-bench fleet). The TUI renders them as one collapsible `backtests` group at the bottom of the sidebar (`tui/src/app/backtests.rs`): runs sharing a label stem (label minus its last `-<segment>`) fold into a single fleet row, per-run rows show state glyph + runtime + VM, Space/Enter on a row toggles the fold (no PTY behind these rows, so plain keys are free), A-i peeks detail incl. the result pointer. Terminal runs leave the group on a grace timer (2 min done / 30 min failed, `blocked_at`-anchored) — shorter than the planning-board archive sweep, since `get_backtest_result` + GCS keep results readable forever. Reconcile also sweeps empty cloud workspaces no live non-backtest task claims (the pre-fix leftovers). A FAILED run's VM is torn down `CM_BACKTEST_BLOCKED_VM_TTL_SECS` (default 30 min) after the blocked transition — anchored on `blocked_at`, NOT launch+`max_runtime_secs` (a run that failed 2 min into a 12h-limit bench used to idle its c2-standard-4 for the rest of the limit) — and the failure evidence survives the VM: the pipeline tees into `/var/log/cm-pipeline.log`, and the failure branch parks it at `<gcs_prefix>/pipeline.log` and merges `exit_code` + `log_tail` into the artifact summary.
...
ssh_user = "lucas"
remote_socket = "/home/lucas/.cm/daemon.sock"
# optional — otherwise fetched from cm-manager:~/.cm/operator-token over ssh:
# operator_token = "…"            # inline
# operator_token_file = "~/.cm/manager-operator-token"
```
...
**Operator token is per host.** Every daemon validates `Caller::Operator` frames against the `CM_OPERATOR_TOKEN` it was started with, and a remote daemon's token is unrelated to the local `~/.cm/operator-token` (cm-manager's `cm-redeploy` minted its own on 2026-08-19 and feeds it to `cm-daemon.service` via an `EnvironmentFile` drop-in). The TUI resolves the token to present per host (`HostPool::operator_token_for`): `operator_token = "…"` inline on the `[[host]]` → `operator_token_file = "~/path"` (local file) → for `ssh-unix` hosts with neither, it fetches `<dirname(remote_socket)>/operator-token` once over the same ssh alias and caches it (a failed fetch logs, falls back to the local token, and retries after 60s) → the local token. Pre-fix the TUI presented its LOCAL token everywhere, so every gated RPC to `manager` (`manifest.watch`, `events.subscribe`, `session.set_workflow_context`, `task.update_tree`, …) failed `operator token does not match` while the ungated ones (`session.list` adopt poll, `attach.open`) kept working — remote agent sessions appeared but their exit diffs never arrived, and each killed worker lingered as a `●` row in its own `agent: <label>` workspace (38 stale rows, 32 of them `detective-*`, on 2026-08-24). Two catch-ups make that class of gap self-healing regardless of the stream: an agent-spawned, non-workflow row is now pruned when its **attach stream** reports a genuine exit (not only on the `Exited` diff), and every `manifest.watch` (re)connect **reconciles against the snapshot** — agent-spawned rows the host no longer lists (or lists as exited) are pruned, exempting rows younger than 10s, user-owned rows, workflow participants, and an A-R restart's own kill. And a restore-deferred agent-spawned entry whose reachable daemon keeps answering `NotFound` is, at the deferred-reattach give-up cap, **tombstoned and dropped from `skipped_manifest_entries`** instead of "preserved in skipped" forever — a given-up entry used to pin its empty `agent: <label>` header open for good via `reap_spent_workspaces`'s attach-pending exemption (the post-restart leftovers). User-owned entries keep the preserve behavior. **Kill-time marker close** (`close_agent_marker_if_empty`): every one of those removal paths (Exited diff, attach-stream exit, snapshot reconcile, give-up settle) soft-closes the adoption-minted `agent: <label>` workspace the instant its last agent session is gone — it is not left for the sweep, and the sweep's task-liveness gate (a subtask still `running`) does NOT hold it open. Scoped strictly to the `agent:` prefix, never cloud workspaces, never a marker with a live row or a pending attach; user-created workspaces (including deliberately empty local ones) are untouched and still go through the sweep's rules.
